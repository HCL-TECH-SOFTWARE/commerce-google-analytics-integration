...
function ProductDetailsLayout({
  ...
  const addToCart = () => {
    ...
    /**GA360 : send Add to Cart event */
    GADataService.sendAddToCartEvent(currentProdSelect);
    /**GA360 */
  ...
  /**GA360 : Send page view and Page detail view data */
  const breadcrumbs = useSelector(breadcrumbsSelector);
  useEffect(() => {
    if (currentProdSelect && breadcrumbs.length !== 0) {
      GADataService.sendPDPPageViewEvent();
      GADataService.sendPDPDetailViewEvent(currentProdSelect);
    }
