...
**/
//Standard libraries
import React, { useEffect, ChangeEvent, MouseEvent, Fragment, useRef } from "react";
...
const ProductGridLayout: React.FC<ProductGridProps> = (props: any) => {
  ...
  /**
   * GA360
   * Send Pageview Event and Product impression Data for datalayer
   **/
    ...
    if (!mounted.current) {
      ...
      if (categoryId === "" && productListTotal >= 0) {
        ...
        if (productListTotal > 0) {
          GADataService.sendProductImpressionEvent(productList, false);
        }
      } else if (categoryId !== "" && productListTotal >= 0) {
        GADataService.sendListerPageViewEvent(productListTotal);
        if (productListTotal > 0) {
          GADataService.sendProductImpressionEvent(productList, true);
        }
      }
    }
  }, [productListTotal]);
